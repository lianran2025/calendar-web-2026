...
type Marks = Record<string, boolean>; // ymd -> marked
  ...
  return (
    <main className="container">
      <div className="header">
        <div>
          <div className="h1">2026 年挂历（双薪 / 三薪标注）</div>
          <div className="sub">
            点击任意日期可“打勾标记”（保存于本地浏览器）。已标记：{counts.total} 天；其中节假日加班日：{counts.specialMarked} 天。
          </div>
      ...
      <div className="footer">
        <div>说明：</div>
        <div>1) 本页面只标注“双薪/三薪”日期（按你提供的 13 天规则），不显示周末/调休。</div>
        <div>2) 标记数据默认只保存在当前设备/浏览器（localStorage）。需要跨设备可用“导出/导入”。</div>
        <div>3) 若以后官方节假日安排或你家规则有变，我可以帮你更新 SPECIAL_DAYS 列表。</div>
      </div>
    </main>
  );
}
